  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Check if wallet was previously connected in MongoDB
        try {
          const walletRes = await fetch('http://localhost:8000/api/wallet/connected', {
            headers: {
      ...
      <div className="flex w-full h-full rounded-2xl overflow-hidden border border-gray-700 shadow-[0_0_0px_rgba(77,126,255,0)] hover:shadow-[0_0_30px_rgba(77,126,255,0.5)] transition-all duration-500 ease-in-out max-w-6xl mx-auto">
        ...
        <div className="w-1/2 h-full flex items-center justify-center bg-[#0A0C10]">
          <div className="w-full max-w-sm p-6">
            ...
            <div className="mb-4 px-4 py-3 rounded-lg bg-[#00F0FF]/5 border border-[#00F0FF]/20 text-center">
              <p className="text-[#00F0FF] text-xs font-semibold mb-1">Test Credentials</p>
              <p className="text-gray-400 text-xs">Email: <span className="text-white">admin@vpx.com</span></p>
              <p className="text-gray-400 text-xs">Password: <span className="text-white">vpx@2024</span></p>
            </div>
